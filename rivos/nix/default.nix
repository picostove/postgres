    ...
    # dependencies
    {
      ...
      in
        stdenv.mkDerivation rec {
          ...
          # RIVOS: we omit man and doc outputs, since these are impure and require
          # network access.
          outputs = ["out" "lib"];
          ...
          ${
            if (instrumentationLinkFlags != [])
            then "NIX_CFLAGS_LINK"
            else null
          } = "${lib.concatStringsSep " " instrumentationLinkFlags}";
          ...
          postFixup =
            ...
            ''
              ...
              wrapProgram $out/bin/postgres \
                          --set LOCALE_ARCHIVE ${glibcLocales}/lib/locale/locale-archive
              wrapProgram $out/bin/pg_ctl \
                          --set LOCALE_ARCHIVE ${glibcLocales}/lib/locale/locale-archive
            '';

          doCheck = !stdenv.isDarwin && !enableM5ops && (stdenv.hostPlatform == stdenv.buildPlatform);
